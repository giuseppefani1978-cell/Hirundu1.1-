...
const label =
bonus.type === 'caffe' ? 'CaffÃ¨ Leccese'
...
// ====== PATCH: CTA victoire + activation bouton A quand bonus dÃ©bloquÃ© ======
function showVictoryBonusCTA() {
// EmpÃªche l'affichage par erreur sur l'accueil si jamais appelÃ©e ailleurs
...
btn.type = 'button';
btn.textContent = 'ðŸŒŸ Victoire : Bonus dÃ©bloquÃ© â†’ Carte';
btn.style.cssText = 'position:fixed;left:50%;bottom:88px;transform:translateX(-50%);z-index:10003;background:#34d399;color:#0b3c2f;border:0;border-radius:14px;padding:12px 16px;font:700 14px/1 system-ui;box-shadow:0 6px 18px rgba(0,0,0,.25);cursor:pointer;';
btn.addEventListener('click', () => {
try {
// compat : on pose 2 clÃ©s
...
const unlocked = (localStorage.getItem('bonus_unlocked') === '1') || (localStorage.getItem('bonus_otranto_unlocked') === '1');
// Essaye divers sÃ©lecteurs possibles du "bouton A"
const btnA = document.querySelector('[data-action="bonus"]')
...
btnA.addEventListener('click', () => window.location.href = MAP_PAGE_URL, { once: true });
} else {
...
ui.showAsk(t.ask?.(poiInfo(key)) || `OÃ¹ est ${poiInfo(key)} ?`);
}
...
const title = won ? (t.win?.() || 'Bravo ! Victoire ðŸŒŸ') : (t.gameover?.() || 'Game Over');
const lines = [
`${title}`,
`Score: ${total} (Ã‰toiles: +${state.score.starsPicked * SCORE.STAR}, Bonus: +${state.score.bonusScore}, Coups: ${state.score.hits * SCORE.HIT}${won ? `, Win: +${SCORE.WIN}` : ''})`,
`Bonus: ${state.score.counts.pasticciotto || 0} Pasticciotto Â· ${state.score.counts.rustico || 0} Rustico Â· ${state.score.counts.caffe || 0} CaffÃ¨`,
`Temps: ${fmtTime(elapsed)}`,
``,
`ðŸ‘‰ check le Hall of Fame en bas du HUD.`,
];
...
stopMusic();
playFinaleLong();
showVictoryBonusCTA(); // PATCH: CTA aprÃ¨s victoire
state.winFx.t = 0;
...
bdText.textContent = 'Conseil: en bataille, â† /â†’ pour bouger, â†‘ pour sauter, A attaquer, B spÃ©cial. Tourne en paysage.';
...
alert('Impossible de charger la battle. Retour Ã  la carte.');
document.body.classList.remove('mode-battle');
state.mode = 'play';
state.running = true;
requestAnimationFrame(draw);
