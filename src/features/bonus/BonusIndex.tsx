...
export default function BonusIndex() {
  ...
  const progressByKey = React.useMemo(() => {
    ...
    progress.forEach((entry) => map.set(entry.key, entry));
    return map;
  }, [progress]);
  ...
  const totals = React.useMemo(() => {
    ...
    return { done: doneCount, unlocked: unlockedCount, total: cards.length };
  }, [cards]);
  ...
  const goHunt = () => {
    ...
    window.location.assign(`${base}#/level/${level}`);
  };

  const backToQR = () => {
    if (typeof window === "undefined") return;
    const base = computeBase();
    window.location.assign(`${base}#/poi/otranto/realmap`);
  };
  ...
  React.useEffect(() => {
    if (!highlightHallOfFame || typeof window === "undefined") return;
    const { pathname, search, hash } = window.location;
    if (!hash.includes("?hof")) return;
  ...
  return (
    <section className="app-section bonus-index__root">
      {/* Topbar façon “Module QR” */}
      <div className="bonus-index__topbar">
        <button
          type="button"
          onClick={backToQR}
          className="app-button app-button--ghost bonus-index__backqr"
          title="Retour au module QR & cartes bonus"
        >
          ↩︎ Retour au module QR
        </button>
        ...
        <div className="bonus-index__actions">
          ...
          >
            🎮 Reprendre la chasse (niv. {nextLevel})
          </button>
          <button
            type="button"
  ...
  return (
    <div className="bonus-index__summary" role="status" aria-live="polite">
      <span className="bonus-index__summary-badge">
        ...
        &nbsp;terminé{totals.done > 1 ? "s" : ""}
      </span>
      <span className="bonus-index__summary-divider" aria-hidden="true">•</span>
      <span>
        {totals.unlocked} carte{totals.unlocked > 1 ? "s" : ""} débloquée{totals.unlocked > 1 ? "s" : ""} sur {totals.total}
      </span>
      {resumeTarget ? (
...
type BonusCardProps = { card: BonusCardModel; onOpen: (key: BonusKey) => void; };
...
type ItineraryProps = { steps: ItineraryStep[]; activeKey?: BonusKey; };
...
type ItineraryCardProps = { step: ItineraryStep; isActive: boolean; };

function ItineraryCard({ step, isActive }: ItineraryCardProps) {
  let statusLabel = "À déverrouiller";
  if (step.completed) statusLabel = "Terminé — rejouer";
  else if (isActive) statusLabel = "Prochaine étape — jouer";
  else if (step.available) statusLabel = "Disponible";
