...
const POIS = [
  ...
  { key:'acaya',            x:0.74, y:0.35 },
  { key:'parco_rauccio',    x:0.78, y:0.22 },
  { key:'san_cataldo',      x:0.85, y:0.28 },
  { key:'cavallino',        x:0.39, y:0.36 },
  { key:'rudiae',           x:0.48, y:0.30 },
  { key:'punta_prosciutto', x:0.09, y:0.34 },
  { key:'porto_selvaggio',  x:0.19, y:0.46 },
  { key:'galatone',         x:0.37, y:0.45 },
  { key:'calimera',         x:0.69, y:0.42 },
];
...
// ----- HOF local (clé dédiée L3) -----
const HOF_KEY = 'salento_hof_v3';
const HOF_SIZE = 10;
  ...
  panel.innerHTML = `
    <div style="height:100%;max-width:900px;margin:0 auto;display:flex;flex-direction:column;padding:16px">
      <div style="display:flex;align-items:center;gap:12px;justify-content:space-between">
        <h2 style="margin:0;font:600 22px system-ui">🏆 Hall of Fame — Niveau 3</h2>
...
function renderHofTable(list){
  ...
  box.innerHTML = `
    ...
    <table>
      <thead>
        <tr>
          <th>#</th><th>Pays</th><th>Joueur</th><th>Score</th><th>Feuilles</th>
    ...
    startBattleIntro({
      ...
      onProceed: async () => {
        try {
          ...
          let modUrl = isDev ? './game_battle_lecce.js' : `./game_battle_lecce.js?v=${APP_VERSION}`;
          let mod;
          try {
...
function drawLeaf(ctx, cx, cy, R){
  ...
  ctx.strokeStyle='rgba(0,0,0,0.25)'; ctx.lineWidth=Math.max(1, R*0.08); ctx.stroke();
  ctx.restore();
}
