...
export function startLevel9(options = {}) {
  ...
  return bootHunt({ ...options, region: {
    ...
    inventoryLabel: ["Cristaux","Cristalli","Crystals","Cristales"],
    bossName: 'Calcara', bossSprite: withBase('assets/boss-8.svg'),
    backdrop: withBase('assets/battle_bg_ostuni.webp'),
    pois: region.places.map((p,i)=>({key:`itria-${i}`,name:p.name,town:p.town,icon:p.icon,clue:p.clue,x:positions[i][0],y:positions[i][1]})),
  }});
}
