...
export function startLevel8(options = {}) {
  ...
  return bootHunt({ ...options, region: {
    ...
    title: region.title,
    subtitle: ['Dix lieux réels, dix découvertes, une bataille.','Dieci luoghi reali, dieci scoperte, una battaglia.','Ten real places, ten discoveries, one battle.','Diez lugares reales, diez descubrimientos, una batalla.'],
    ...
    inventoryLabel: ["Amphores","Anfore","Amphorae","Ánforas"],
    bossName: 'Argillo', bossSprite: withBase('assets/boss-9.svg'),
    backdrop: withBase('assets/battle_bg_messapia.webp'),
    pois: region.places.map((p,i)=>({key:`messapia-${i}`,name:p.name,town:p.town,icon:p.icon,clue:p.clue,x:positions[i][0],y:positions[i][1]})),
  }});
}
