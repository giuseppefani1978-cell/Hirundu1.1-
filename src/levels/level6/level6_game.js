...
export function startLevel6(options = {}) {
  ...
  return bootHunt({ ...options, region: {
    ...
    title: region.title,
    subtitle: ['Dix lieux réels, dix pins, une bataille.','Dieci luoghi reali, dieci pini, una battaglia.','Ten real places, ten pines, one battle.','Diez lugares reales, diez pinos, una batalla.'],
    ...
    inventoryLabel: ['Pins','Pini','Pines','Pinos'],
    bossName: 'Resino', bossSprite: withBase('assets/boss-6.svg'),
    backdrop: withBase('assets/battle_bg_copertino.webp'),
    pois: region.places.map((p,i)=>({key:`arneo-${i}`,name:p.name,town:p.town,icon:p.icon,clue:p.clue,x:positions[i][0],y:positions[i][1]})),
  }});
}
