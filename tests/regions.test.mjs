...
test('regional content, progression and reset preserve the nine-level journey',async()=>{
 ...
 try{
  ...
  assert.equal(store.getProgressList().length,9);
  for(const n of [1,2,3,4,5,6,7,8]){store.markLevelWin(n);assert.equal(store.getNextLevel().id,n+1);}
  store.markLevelWin(9);assert.equal(store.getNextLevel(),undefined);
