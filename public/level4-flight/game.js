...
function drawScrollingCoast(bg){if(!bg?.naturalWidth||!bg?.naturalHeight)return;const k=Math.max(W/bg.naturalWidth,(H*1.18)/bg.naturalHeight),dw=bg.naturalWidth*k,dh=bg.naturalHeight*k,x=(W-dw)/2,off=S.offset%dh;for(let y=off-dh;y<H;y+=dh)ctx.drawImage(bg,x,y,dw,dh+1);}
function draw(){ctx.clearRect(0,0,W,H);ctx.fillStyle='#369fab';ctx.fillRect(0,0,W,H);const bg=images.coast;if(bg?.naturalWidth)drawScrollingCoast(bg);
...
function launchBattle(){
  ...
  const returnUrl=location.origin+rootPath+'level4-flight/';
  try{
  ...
  location.href=location.origin+rootPath+'#/level/4?test=battle&from=flight';
}
