...
// side-view movement/jump, normal A, inventory-powered B, Scirocco wind blasts.
const Battle={
 ...
 start(){
  ...
  this.data={time:0,stage:'entry',stageTime:1.5,phase:1,player:{x:155,y:420,vx:0,vy:0,hp:120,facing:1,inv:0,flapCooldown:0,assist:0,targetVy:0,dodge:0,dodgeCooldown:0,dodgeDir:1},boss:{x:1040,y:420,hp:240,flash:0},ammo,shots:[],fx:[],cooldown:0,combo:0,comboTime:0,shake:0,finishTime:0,fireTimer:0};
  state.mode='battle';keys.clear();state.message=0;$('cover').classList.add('hidden');$('message').classList.remove('visible');document.body.classList.add('battle-active');$('battleHUD').hidden=false;$('speaker').textContent=extra().battle;$('question').textContent=battleUI().help;$('targetsList').textContent='Aracne ✦ Scirocco';$('tip').textContent=extra().orientation;resize();this.hud();canvas.focus({preventScroll:true});
 },
 ...
 volley(){const d=this.data,b=d.boss,p=d.player;const dx=p.x-b.x,dy=(p.y-42)-(b.y-58),base=Math.atan2(dy,dx),speed=d.phase===2?370:320;
 ...
 tick(dt){
  ...
  if(b.hp<=120&&d.phase===1){d.phase=2;notice(extra().phase,2);this.burst(b.x,b.y-60,'#f6b756',32)}
  if(d.stageTime<=0){if(d.stage==='patrol')this.setStage('warning',.85);else if(d.stage==='warning'){this.volley();this.setStage('exposed',1.5)}else if(d.stage==='exposed')this.setStage('patrol',d.phase===2?.85:1.55)}
 ...
 draw(bounds){
  ...
  if(d.stage==='entry'){ctx.font='bold 40px Georgia';ctx.textAlign='center';ctx.fillStyle='#fff8d8';ctx.strokeStyle='#745220';ctx.lineWidth=4;ctx.strokeText('Scirocco',480,150);ctx.fillText('Scirocco',480,150)}ctx.restore();
 }
};
