...
function translated(){const t=text();document.documentElement.lang=lang;$('language').value=lang;for(const [id,key] of [['subtitle','subtitle'],['speaker','speaker'],['hint','hint'],['tip','tip'],['cardTag','tag'],['restart','restart']])$(id).textContent=t[key];$('launch').textContent=state.mode==='flying'?t.recall:t.launch;$('launch').title=t.recallTip;question();extraLabels();updatePowerHUD();if(state.mode==='intro')showIntro();else if(state.mode==='paused')showPause();else if(state.mode==='won')showWin();else if(state.mode==='battleIntro')battleIntro();else if(state.mode==='battleLost')Battle.lose();}
function cover(title,description,help,label){const intro=state.mode==='battleIntro';document.body.classList.toggle('battle-intro',intro);$('battlePreview').hidden=!intro;$('battleSupplies').hidden=!intro;$('orientationPrompt').hidden=!intro;$('play').disabled=!state.assetsReady;$('cover').classList.remove('hidden');$('cardTitle').textContent=title;$('cardText').textContent=description;$('cardHelp').textContent=help;$('play').textContent=label;$('restart').hidden=state.mode!=='paused';$('continueLevel').hidden=state.mode!=='won'||trainingBattle;$('discoveries').hidden=state.mode!=='won'||trainingBattle;$('exitLevel').hidden=!['paused','won','intro'].includes(state.mode);$('installApp').hidden=state.mode!=='paused'||!hostInstallAvailable;}
...
function mapViewport(){const f=Math.min(W/1024,H/1536);return {x:(W-1024*f)/2,y:(H-1536*f)/2,w:1024*f,h:1536*f};}
function resize(){
...
function resume(){if(state.previous==='battle'&&!battleOrientationReady()){syncBattleOrientation();return}accumulator=0;state.mode=state.previous;$('cover').classList.add('hidden');last=0;canvas.focus({preventScroll:true});}
...
$('language').onchange=()=>{lang=$('language').value;try{localStorage.setItem('hirundu_arcade_language',lang);localStorage.setItem('__lang__',lang)}catch{}translated();labelBridge()};
