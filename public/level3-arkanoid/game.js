...
function translated(){const t=text();document.documentElement.lang=lang;$('language').value=lang;$('pauseLanguageLabel').textContent=languageLabels[lang]||'Language';for(const [id,key] of [['subtitle','subtitle'],['speaker','speaker'],['hint','hint'],['tip','tip'],['cardTag','tag'],['restart','restart']])$(id).textContent=t[key];$('launch').textContent=state.mode==='flying'?t.recall:t.launch;$('launch').title=t.recallTip;question();extraLabels();updatePowerHUD();if(state.mode==='intro')showIntro();else if(state.mode==='paused')showPause();else if(state.mode==='won')showWin();else if(state.mode==='battleIntro')battleIntro();else if(state.mode==='battleLost')Battle.lose();}
function cover(title,description,help,label){const intro=state.mode==='battleIntro';document.body.classList.toggle('battle-intro',intro);$('battlePreview').hidden=!intro;$('battleSupplies').hidden=!intro;$('orientationPrompt').hidden=!intro;$('pauseLanguage').hidden=state.mode!=='paused';$('play').disabled=!state.assetsReady;$('cover').classList.remove('hidden');$('cardTitle').textContent=title;$('cardText').textContent=description;$('cardHelp').textContent=help;$('play').textContent=label;$('restart').hidden=state.mode!=='paused';$('continueLevel').hidden=state.mode!=='won'||trainingBattle;$('discoveries').hidden=state.mode!=='won'||trainingBattle;$('exitLevel').hidden=!['paused','won','intro'].includes(state.mode);$('installApp').hidden=state.mode!=='paused'||!hostInstallAvailable;}
