...
// Level 3 uses the former Slow setting. Higher speeds belong to later levels.
const BIRD_SPEED=HUNT_MODEL.birdSpeed;
const POWERS=HUNT_MODEL.powers;
let H=820,PY=752;
const mapPoints={lecce:[.53,.28],acaya:[.74,.35],parco_rauccio:[.78,.22],san_cataldo:[.85,.28],cavallino:[.39,.36],rudiae:[.48,.30],punta_prosciutto:[.09,.34],porto_selvaggio:[.19,.46],galatone:[.37,.45],calimera:[.69,.42]};
const ids=['lecce','acaya','parco_rauccio','san_cataldo','cavallino','rudiae','punta_prosciutto','porto_selvaggio','galatone','calimera'];
const ui={
fr:{subtitle:'NIVEAU 3 · LE VOL DES FEUILLES',speaker:'Tarantula',ask:'Où trouver',title:'Vise la bonne découverte.',intro:'La tarentule pose une énigme. Fais rebondir Aracne vers le lieu qui répond à sa question.',help:'Glisse le doigt pour déplacer la plateforme. Ses bords orientent le rebond. Touche le terrain pour lancer.',play:'Jouer',launch:'Envol ↗',hint:'Indice',pace:'Rythme',speeds:['Doux','Vif','Rapide'],tip:'Glisse pour diriger · touche pour lancer · espace / ← →',ready:'À toi ! Déplace la plateforme puis lance Aracne.',wrong:'Pas ce lieu. Cherche encore !',found:'Découverte !',retry:'Aracne revient. Relance depuis la plateforme !',pause:'Le vol est en pause',resume:'Reprendre',restart:'Recommencer',win:'Les 10 lieux retrouvés !',won:'Le Sceau de Lecce rejoint ta collection dans ce navigateur.',score:'points',hintText:'Vise',loading:'Chargement…',error:'Les images ne se sont pas chargées. Réessaie.',reload:'Réessayer',again:'Rejouer',tag:'LE VOL D’ARACNE',paused:'Prends ton temps. La question et ta progression restent là.',bricks:'🍃 +5',collection:'Sceau de Lecce',recall:'Retour d’Aracne',recallTip:'Rappelle Aracne sur la plateforme pour viser à nouveau.'},
en:{subtitle:'LEVEL 3 · FLIGHT OF THE LEAVES',speaker:'Tarantula',ask:'Where can you find',title:'Aim for the right discovery.',intro:'The tarantula gives a clue. Bounce Aracne towards the place that answers her question.',help:'Slide to move the paddle. Its edges change your rebound angle. Tap the field to launch.',play:'Play',launch:'Launch ↗',hint:'Hint',pace:'Pace',speeds:['Gentle','Lively','Fast'],tip:'Slide to steer · tap to launch · space / ← →',ready:'Your turn! Move the paddle, then launch Aracne.',wrong:'Not that place. Try another!',found:'Discovered!',retry:'Aracne is back. Launch again from the paddle!',pause:'Flight paused',resume:'Resume',restart:'Restart',win:'All 10 places discovered!',won:'The Lecce Seal joins your collection in this browser.',score:'points',hintText:'Aim for',loading:'Loading…',error:'The images could not load. Please retry.',reload:'Retry',again:'Play again',tag:'ARACNE’S FLIGHT',paused:'Take your time. Your clue and progress are waiting.',bricks:'🍃 +5',collection:'Lecce Seal',recall:'Recall Aracne',recallTip:'Bring Aracne back to the paddle to aim again.'},
it:{subtitle:'LIVELLO 3 · IL VOLO DELLE FOGLIE',speaker:'Tarantula',ask:'Dove si trova',title:'Mira alla scoperta giusta.',intro:'La tarantola propone un enigma. Fai rimbalzare Aracne verso il luogo che risponde alla domanda.',help:'Scorri per muovere la piattaforma. I bordi cambiano la direzione del rimbalzo. Tocca il campo per partire.',play:'Gioca',launch:'Vola ↗',hint:'Indizio',pace:'Ritmo',speeds:['Dolce','Vivace','Veloce'],tip:'Scorri per dirigere · tocca per partire · spazio / ← →',ready:'Muovi la piattaforma e fai volare Aracne!',wrong:'Non è questo luogo. Riprova!',found:'Scoperto!',retry:'Aracne è tornata. Riparti dalla piattaforma!',pause:'Il volo è in pausa',resume:'Riprendi',restart:'Ricomincia',win:'Tutti i 10 luoghi scoperti!',won:'Il Sigillo di Lecce entra nella tua collezione in questo browser.',score:'punti',hintText:'Mira a',loading:'Caricamento…',error:'Immagini non caricate. Riprova.',reload:'Riprova',again:'Rigioca',tag:'IL VOLO DI ARACNE',paused:'Prenditi il tuo tempo. La domanda e i progressi ti aspettano.',bricks:'🍃 +5',collection:'Sigillo di Lecce',recall:'Richiama Aracne',recallTip:'Riporta Aracne sulla piattaforma per mirare di nuovo.'},
es:{subtitle:'NIVEL 3 · EL VUELO DE LAS HOJAS',speaker:'Tarantula',ask:'¿Dónde está',title:'Apunta al descubrimiento correcto.',intro:'La tarántula propone un enigma. Haz rebotar a Aracne hacia el lugar que responde a su pregunta.',help:'Desliza para mover la plataforma. Sus bordes cambian el ángulo del rebote. Toca el campo para despegar.',play:'Jugar',launch:'Vuela ↗',hint:'Pista',pace:'Ritmo',speeds:['Suave','Ágil','Rápido'],tip:'Desliza para dirigir · toca para lanzar · espacio / ← →',ready:'Mueve la plataforma y lanza a Aracne.',wrong:'No es ese lugar. ¡Prueba otro!',found:'¡Descubierto!',retry:'Aracne ha vuelto. ¡Lánzala de nuevo!',pause:'Vuelo en pausa',resume:'Continuar',restart:'Reiniciar',win:'¡Los 10 lugares descubiertos!',won:'El Sello de Lecce se añade a tu colección en este navegador.',score:'puntos',hintText:'Apunta a',loading:'Cargando…',error:'No se cargaron las imágenes. Inténtalo de nuevo.',reload:'Reintentar',again:'Volver a jugar',tag:'EL VUELO DE ARACNE',paused:'Tómate tu tiempo. Tu pregunta y progreso te esperan.',bricks:'🍃 +5',collection:'Sello de Lecce',recall:'Llamar a Aracne',recallTip:'Devuelve a Aracne a la plataforma para apuntar otra vez.'}
};
...
function ensureLevelTrack(kind){
 ...
 if(kind==='battle')battleMusic=track;else huntMusic=track;
 return track;
}
function pauseLevelMusic(){
 for(const track of [huntMusic,battleMusic]){try{track?.pause()}catch{}}
}
function musicKindForMode(){
 const mode=state.mode==='paused'?state.previous:state.mode;
 if(mode==='battle')return 'battle';
 if(['ready','flying','transition'].includes(mode))return 'hunt';
 return null;
}
function playLevelMusic(kind=musicKindForMode()){
...
function setLevelMusicEnabled(enabled){
 ...
 try{localStorage.setItem(MUSIC_PREF_KEY,musicEnabled?'on':'off')}catch{}
 if(musicEnabled)playLevelMusic();else pauseLevelMusic();
 updatePauseMusicLabel();
}
function toggleLevelMusic(){setLevelMusicEnabled(!musicEnabled)}
function updatePauseMusicLabel(){const labels=pauseMusicLabels[lang]||pauseMusicLabels.en;$('pauseMusic').textContent=musicEnabled?labels.on:labels.off;}
...
function resize(){
 ...
 if(oldH!==H){
  ...
  state.bricks.forEach((brick,i)=>brick.y=H*.58+Math.floor(i/8)*31);
...
function pause(){touchPointer=null;if(!['ready','flying','transition','battle'].includes(state.mode))return;state.previous=state.mode;state.mode='paused';keys.clear();pauseLevelMusic();showPause();}
function resume(){if(state.previous==='battle'&&!battleOrientationReady()){syncBattleOrientation();return}accumulator=0;state.mode=state.previous;document.body.classList.remove('pause-active');$('cover').classList.add('hidden');last=0;playLevelMusic();canvas.focus({preventScroll:true});}
...
function finish(){pauseLevelMusic();state.mode='won';sendHost('victory',{score:state.score,leaves:state.found.length,elapsed:state.elapsed,misses:state.misses,food:{...state.food}});state.message=0;$('message').classList.remove('visible');try{if(!trainingBattle)localStorage.setItem('hirundu_l3_seal','lecce');if(!trainingBattle)localStorage.setItem('hirundu_arcade_l3_best',String(Math.max(state.score,Number(localStorage.getItem('hirundu_arcade_l3_best'))||0)))}catch{}showWin()}
...
async function loadAssets(){try{await Promise.all(['map','bird','tarantula','battle','boss','coffee','rustico','pasticciotto','crow','jelly'].map(name=>new Promise((resolve,reject)=>{const img=new Image();images[name]=img;img.onload=resolve;img.onerror=reject;img.src={"bird": "../assets/aracne%20.PNG", "map": "../assets/salento-map.PNG", "tarantula": "../assets/tarantula%20.PNG", "coffee": "../assets/caffeleccese%20.PNG", "rustico": "../assets/rustico.PNG", "pasticciotto": "../assets/bonus-pasticciotto.PNG", "boss": "../assets/sputacchina_boss.png", "battle": "../assets/battle_bg_lecce.png", "crow": "../assets/crow.PNG", "jelly": "../assets/jellyfish.PNG"}[name]})));state.assetsReady=true;$('play').disabled=false;if(trainingBattle){state.found=ids.slice();state.food={coffee:1,rustico:1,pasticciotto:1};hud();battleIntro()}else showIntro()}catch{$('cardText').textContent=text().error;$('play').textContent=text().reload;$('play').disabled=false;$('play').onclick=()=>location.reload()}}

const extras={
fr:{intro:'Brise les murs de pierre pour accéder aux lieux de la carte. La tarentule te donne l’indice : guide Aracne vers le bon repère numéroté.',wall:'Murs de pierre',coffee:'Caffè leccese · énergie +30',rustico:'Rustico · bouclier 8 s',pasticciotto:'Pasticciotto · énergie +20 · vol ralenti 7 s',hit:'Attention aux ennemis !',energy:'Énergie',battle:'Bataille de Lecce',battleIntro:'Les dix lieux sont retrouvés. Affronte la Sputacchina avec les provisions récoltées.',battleHelp:'← → bouger · ↑ sauter · A attaquer · B spécial. Les provisions alimentent les attaques spéciales.',fight:'Combattre',jump:'Saut',attack:'Attaque',special:'Spécial',warning:'Attention : salve de spores !',exposed:'Maintenant ! Le boss est vulnérable.',phase:'La Sputacchina accélère !',lost:'Aracne reprend son souffle',retry:'Réessayer la bataille',lostText:'Tes découvertes et tes provisions sont conservées pour réessayer.',victory:'Lecce libérée !',boss:'Sputacchina',safe:'Bouclier actif',focus:'Vol ralenti',normal:'Vol calme',orientation:'Bataille : le mode paysage offre plus de place.',victoryText:'Victoire sur la Sputacchina ! Le Sceau de Lecce est conservé dans ce navigateur.'},
en:{intro:'Break through the stone walls to reach the places on the map. Follow the tarantula’s clue and guide Aracne into the right numbered marker.',wall:'Stone walls',coffee:'Caffè leccese · energy +30',rustico:'Rustico · shield 8 s',pasticciotto:'Pasticciotto · energy +20 · slower flight 7 s',hit:'Watch out for enemies!',energy:'Energy',battle:'Battle of Lecce',battleIntro:'All ten places found. Face the Sputacchina with the provisions you collected.',battleHelp:'← → move · ↑ jump · A attack · B special. Collected food powers special attacks.',fight:'Fight',jump:'Jump',attack:'Attack',special:'Special',warning:'Incoming spore volley!',exposed:'Now! The boss is vulnerable.',phase:'The Sputacchina speeds up!',lost:'Aracne catches her breath',retry:'Retry battle',lostText:'Your discoveries and provisions are kept for the retry.',victory:'Lecce is free!',boss:'Sputacchina',safe:'Shield active',focus:'Slow flight',normal:'Easy flight',orientation:'Battle: landscape gives you more space.',victoryText:'The Sputacchina is defeated! The Lecce Seal is saved in this browser.'},
it:{intro:'Rompi i muri di pietra per raggiungere i luoghi sulla mappa. Segui l’indizio della tarantola e guida Aracne sul segnaposto giusto.',wall:'Muri di pietra',coffee:'Caffè leccese · energia +30',rustico:'Rustico · scudo 8 s',pasticciotto:'Pasticciotto · energia +20 · volo lento 7 s',hit:'Attenzione ai nemici!',energy:'Energia',battle:'Battaglia di Lecce',battleIntro:'Hai trovato i dieci luoghi. Affronta la Sputacchina con le provviste raccolte.',battleHelp:'← → muovi · ↑ salta · A attacco · B speciale. Le provviste alimentano gli attacchi speciali.',fight:'Combatti',jump:'Salto',attack:'Attacco',special:'Speciale',warning:'Attenzione: raffica di spore!',exposed:'Ora! Il boss è vulnerabile.',phase:'La Sputacchina accelera!',lost:'Aracne riprende fiato',retry:'Riprova la battaglia',lostText:'Le scoperte e le provviste restano disponibili per riprovare.',victory:'Lecce è libera!',boss:'Sputacchina',safe:'Scudo attivo',focus:'Volo lento',normal:'Volo tranquillo',orientation:'Battaglia: in orizzontale hai più spazio.',victoryText:'La Sputacchina è sconfitta! Il Sigillo di Lecce è salvato in questo browser.'},
es:{intro:'Rompe los muros de piedra para alcanzar los lugares del mapa. Sigue la pista de la tarántula y guía a Aracne al marcador correcto.',wall:'Muros de piedra',coffee:'Caffè leccese · energía +30',rustico:'Rustico · escudo 8 s',pasticciotto:'Pasticciotto · energía +20 · vuelo lento 7 s',hit:'¡Cuidado con los enemigos!',energy:'Energía',battle:'Batalla de Lecce',battleIntro:'Has encontrado los diez lugares. Enfréntate a la Sputacchina con las provisiones recogidas.',battleHelp:'← → mover · ↑ saltar · A atacar · B especial. La comida alimenta los ataques especiales.',fight:'Combatir',jump:'Salto',attack:'Ataque',special:'Especial',warning:'¡Atención: ráfaga de esporas!',exposed:'¡Ahora! El jefe es vulnerable.',phase:'¡La Sputacchina acelera!',lost:'Aracne recupera el aliento',retry:'Reintentar batalla',lostText:'Conservas tus descubrimientos y provisiones para reintentar.',victory:'¡Lecce es libre!',boss:'Sputacchina',safe:'Escudo activo',focus:'Vuelo lento',normal:'Vuelo tranquilo',orientation:'Batalla: en horizontal tienes más espacio.',victoryText:'¡Sputacchina derrotada! El Sello de Lecce se guarda en este navegador.'}
};
function extra(){return extras[lang]}
function extraLabels(){ui[lang].intro=extra().intro;ui[lang].win=extra().victory;ui[lang].won=extra().victoryText;$('jump').textContent='↑';$('jump').setAttribute('aria-label',battleUI().flap);$('dive').setAttribute('aria-label',battleUI().dive);$('dodge').textContent='↯ '+battleUI().dodge;$('attack').textContent='A · '+extra().attack;$('special').textContent='B · '+extra().special;$('energyLabel').textContent=extra().energy;$('bonusHelp').textContent=extra().coffee+' · '+extra().rustico+' · '+extra().pasticciotto;for(const kind of ['coffee','rustico','pasticciotto'])$(kind+'Count').parentElement?.setAttribute('title',extra()[kind]);$('targetsList').textContent=state.targets.map(t=>t.number+' · '+place(t.id).name).join('   /   ');if(state.mode==='battle'||(state.mode==='paused'&&state.previous==='battle')){Battle.hud();$('speaker').textContent=extra().battle;$('question').textContent=extra().battleHelp;$('targetsList').textContent='Aracne ✦ Sputacchina';$('tip').textContent=extra().orientation;}}
...
function sendHost(type,data={}){if(window.parent!==window)window.parent.postMessage({source:'hirundu-level3',type,...data},location.origin)}
...
for(const [id,type] of [['continueLevel','continue'],['discoveries','discoveries'],['exitLevel','exit'],['installApp','install']])$(id).onclick=()=>sendHost(type);
$('pauseMusic').onclick=()=>toggleLevelMusic();
addEventListener('message',e=>{if(e.source!==window.parent||e.origin!==location.origin||e.data?.source!=='hirundu-host')return;if(e.data.type==='install-state'){hostInstallAvailable=!!e.data.available;$('installApp').hidden=state.mode!=='paused'||!hostInstallAvailable}if(e.data.type==='language'&&ui[e.data.lang]){const wasPaused=state.mode==='paused',pausedFrom=state.previous;lang=e.data.lang;translated();labelBridge();if(wasPaused){state.previous=pausedFrom;state.mode='paused';showPause()}}if(e.data.type==='pause')pause()});
...
if(document.modelContext?.registerTool){const lifecycle=new AbortController();try{Promise.resolve(document.modelContext.registerTool({name:'read_level3_progress',description:'Read progress in the Aracne rebound prototype.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>({mode:state.mode,discoveries:state.found.length,score:state.score,question:state.round<10?place(active()).info:null,inventory:{...state.food},battle:state.mode==='battle'?{playerHP:Battle.data.player.hp,bossHP:Battle.data.boss.hp}:null})},{signal:lifecycle.signal})).catch(()=>{})}catch{}addEventListener('pagehide',()=>lifecycle.abort(),{once:true})}
