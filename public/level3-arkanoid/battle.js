...
// side-view movement/jump, normal A, inventory-powered B, Sputacchina spores.
const Battle={
 ...
 volley(){const d=this.data,b=d.boss,p=d.player;const dx=p.x-b.x,dy=(p.y-42)-(b.y-58),base=Math.atan2(dy,dx),speed=d.phase===2?335:290;
 ...
 tick(dt){
  ...
  if(b.hp<=100&&d.phase===1){d.phase=2;notice(extra().phase,2);this.burst(b.x,b.y-60,'#f6b756',32)}
  if(d.stageTime<=0){if(d.stage==='patrol')this.setStage('warning',.85);else if(d.stage==='warning'){this.volley();this.setStage('exposed',1.5)}else if(d.stage==='exposed')this.setStage('patrol',d.phase===2?1:1.8)}
 ...
 draw(bounds){
  ...
  if(d.stage==='entry'){ctx.font='bold 40px Georgia';ctx.textAlign='center';ctx.fillStyle='#fff8d8';ctx.strokeStyle='#745220';ctx.lineWidth=4;ctx.strokeText('Sputacchina',480,150);ctx.fillText('Sputacchina',480,150)}ctx.restore();
 }
};
